...
var predictPartyVictory = function(senate) {
    ...
    while(r.length && d.length) {
        let radient=r.shift();
        let dire = d.shift();
        if(radient<dire) r.push(radient+n)
        else d.push(dire+n)
    }
    return r.length > 0 ? 'Radiant' :'Dire'
};

console.log("victory for RD : ",predictPartyVictory("RD"));
console.log("victory for RDD : ",predictPartyVictory("RDD"));
